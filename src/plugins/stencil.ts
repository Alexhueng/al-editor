import { Dom, FunctionExt, CssLoader, Cell, Node, Model, View, Graph } from '@antv/x6'
import type { EventArgs } from '@antv/x6'
import { Dnd } from '@antv/x6-plugin-dnd'
import { grid } from '@antv/x6-plugin-stencil/es/grid'
import { content } from '@antv/x6-plugin-stencil/es/style/raw'
...
export class Stencil extends View implements Graph.Plugin {
  ...
  protected loadGroup(cells: (Node | Node.Metadata)[], groupName?: string, reverse?: boolean) {
  ...
  protected onDragStart(args: EventArgs['node:mousedown']) {
    ...
    let cell = node
    if (this.options.beforeDndStart) {
      cell = this.options.beforeDndStart({
        node,
        e,
      })
    }
    this.dnd.start(cell, e)
  }
