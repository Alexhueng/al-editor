import { GeometryUtil, Angle, Point, Dom, NumberExt, Node, Graph, View, NodeView } from '@antv/x6'
...
export class TransformImpl extends View<TransformImpl.EventArgs> {
  ...
  protected updateResizerDirections() {
    ...
    if (shift !== this.prevShift) {
      ...
      const className = (dir: string) => `${this.containerClassName}-cursor-${dir}`
  ...
  protected prepareResizing(evt: Dom.EventObject, relativeDirection: Node.ResizeDirection) {
  ...
  protected stopHandle() {
    ...
    if (this.handle) {
      ...
      const pos = this.handle.getAttribute('data-position') as Node.ResizeDirection
      if (pos) {
        const dir = Private.DIRECTIONS[Private.POSITIONS.indexOf(pos)]
  ...
  protected notify<K extends keyof TransformImpl.EventArgs, T extends Dom.EventObject>(
    name: K,
    evt: T,
    view: NodeView,
    args: KeyValue = {},
